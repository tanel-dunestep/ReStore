...
export default function BasketPage() {
	...
	function handleAddItem(name: string, productId: number) {
	...
	function handleRemoveItem(name: string, productId: number, quantity: number = 1) {
		setStatus({ loading: true, name });
	...
	return (
		<>
							...
							<TableRow key={item.productId} sx={{ "&:last-child td, &:last-child th": { border: 0 } }}>
								...
								<TableCell align="right">
									<LoadingButton
										...
										color="error"
										onClick={() =>
											handleRemoveItem("delete" + item.productId, item.productId, item.quantity)
										}
									>
										<Delete />
									</LoadingButton>
			...
			<Grid container>
				...
				<Grid item xs={6}>
					<BasketSummary />
					<Button component={Link} to="/checkout" color="primary" size="large" variant="contained" fullWidth>
						Checkout
					</Button>
				</Grid>
			</Grid>
